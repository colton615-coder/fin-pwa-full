...
const net = (arr) => arr.reduce((s,t)=>s+t.amount*(t.type==='expense'?-1:1),0);
...
export default function Dashboard() {
  ...
  const spentWeek = useMemo(() => net(weekItems), [weekItems]);
  const spentMonth = useMemo(() => net(monthItems), [monthItems]);
  ...
  const monthSpending = Math.max(0, -spentMonth);
  const remaining = Math.max(0, (budget || 0) - monthSpending);
  const over = budget > 0 && monthSpending > budget;
      ...
      <div className="card">
        <div className="row" style={{justifyContent:'space-between'}}>
          <div>
            ...
            <div className="title mono">{fmtMoney(Math.max(0, -spentWeek), currency)}</div>
          </div>
          <div>
            <div className="small">This Month</div>
            <div className="title mono">{fmtMoney(monthSpending, currency)}</div>
          </div>
        </div>
